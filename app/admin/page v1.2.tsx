...
import { 
  ...
  RefreshCw, ClipboardPaste, Trash2, Plus, Image as ImageIcon, Copy, Check, X, FolderOpen 
} from "lucide-react";
...
const ImageUploadModal = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  const copyAllToClipboard = () => {
    ...
    setIsAllCopied(true);
    setTimeout(() => setIsAllCopied(null), 2000);
  };
    ...
    const processQueue = async () => {
      ...
      try {
        ...
        setQueue(prev => prev.map(item => 
          item.id === nextItem.id 
            ? { ...item, status: 'success', url: publicUrl } 
            : item
        ));
      } catch (err: any) {
        setQueue(prev => prev.map(item => 
...
export default function AdminDashboardPage() {
      ...
      handlePaste: (pastedData: any[][], startRowIndex: number, startColIndex: number) => {
        setRecords(old => {
          ...
          pastedData.forEach((pastedRow, i) => {
            ...
            if (rowIndex >= newData.length) newData.push({}); // 필요 시 새 행 생성
  ...
  useEffect(() => {
    const handlePasteEvent = (event: ClipboardEvent) => {
      ...
      const colIdx = (td as HTMLTableCellElement).cellIndex - 1; // # 번호 열 제외
  ...
  return (
    <div className="flex h-screen w-full bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 overflow-hidden">
      ...
      {/* --- Sidebar (기존 기능 100% 유지 + 이미지 버튼 추가) --- */}
          ...
          <>
            <header className="h-16 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-black px-6 flex items-center justify-between shrink-0 shadow-sm z-10">
              <div className="flex items-center gap-4">
                ...
                <button onClick={handleAddColumn} disabled={isAddingColumn} className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-300 rounded-lg text-xs font-bold transition-all border border-zinc-200 dark:border-zinc-700">
                  {isAddingColumn ? <Loader2 className="w-3 h-3 animate-spin" /> : <Plus className="w-3 h-3" />}
                  컬럼 추가
                </button>
              </div>
              <div className="flex items-center gap-2">
                  ...
                  <tbody>
                    {table.getRowModel().rows.map((row, i) => (
                      <tr key={row.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30">
                        <td className="p-2 border-r dark:border-zinc-800 text-center text-zinc-400">{i + 1}</td>
