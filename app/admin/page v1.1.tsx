...
import { Database, Table, Columns, Key, Settings, Search, Loader2, Save, RefreshCw, ClipboardPaste, Trash2, Plus } from "lucide-react";
import {
  flexRender,
  getCoreRowModel,
  useReactTable,
...
export default function AdminDashboardPage() {
  ...
  const handleCreateTable = async () => {
    ...
    const { error } = await supabase.rpc("create_new_table", {
  ...
  const handleAddColumn = async () => {
    ...
    if (error) {
      alert(`컬럼 추가 실패: ${error.message}`);
    } else {
      alert(`'${colName}' 컬럼이 성공적으로 추가되었습니다.`);
      // 스키마와 데이터를 새로고침하여 즉시 반영
      window.location.reload(); 
    }
      ...
      <aside className="w-64 border-r border-zinc-200 dark:border-zinc-800 bg-white dark:bg-black flex flex-col shrink-0">
        <div className="p-5 flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800">
          ...
          <button 
            ...
            className="p-1.5 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500 transition-colors disabled:opacity-50"
            title="새 테이블 추가"
          >
            ...
            <header className="h-16 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-black px-6 flex items-center justify-between shrink-0 shadow-sm z-10">
              <div className="flex items-center gap-4">
                ...
                <button 
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-300 rounded-lg text-xs font-bold transition-all border border-zinc-200 dark:border-zinc-700"
                >
