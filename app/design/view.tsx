    ...
    <div className={`flex gap-3 p-3 rounded-2xl border-2 border-slate-200 ${depth % 2 === 0 ? 'bg-slate-50' : 'bg-white'} relative mb-3 group/row transition-all min-w-max w-full`}>
      ...
      <div className="absolute -left-3 -top-3 flex items-center bg-indigo-600 text-white rounded-full shadow-lg opacity-0 group-hover/row:opacity-100 z-40 overflow-hidden transition-all border-2 border-white">
        <button onClick={(e) => {
          e.stopPropagation();
          mutate(rows => {
            const findAndUpdate = (arr: any[]) => {
              for (const r of arr) {
                if (r.id === row.id) { r.flex = Math.max(1, (r.flex || 1) - 1); return true; }
                for (const c of r.cells) if (c.nestedRows && findAndUpdate(c.nestedRows)) return true;
              }
              return false;
            };
            findAndUpdate(rows);
          });
        }} className="p-1 hover:bg-indigo-700 transition-colors" title="세로 비율 줄이기"><Minus size={14} strokeWidth={3}/></button>
        ...
        <button onClick={(e) => {
          ...
          mutate(rows => {
            const findAndUpdate = (arr: any[]) => {
              for (const r of arr) {
                if (r.id === row.id) { r.flex = (r.flex || 1) + 1; return true; }
                for (const c of r.cells) if (c.nestedRows && findAndUpdate(c.nestedRows)) return true;
              }
              return false;
            };
        ...
        }} className="p-1 hover:bg-indigo-700 transition-colors" title="세로 비율 늘리기"><Plus size={14} strokeWidth={3}/></button>
      </div>
          ...
          <div className="flex justify-between items-center bg-indigo-50 px-2 py-1 rounded-xl">
            <div className="flex items-center gap-1.5">
              <button onClick={() => mutate(rows => {
                ...
                find(rows);
              })} className="text-indigo-700 hover:scale-125 transition-transform p-1" title="가로 비율 줄이기"><ChevronLeft size={16}/></button>
              ...
              <button onClick={() => mutate(rows => {
                ...
                find(rows);
              })} className="text-indigo-700 hover:scale-125 transition-transform p-1" title="가로 비율 늘리기"><ChevronRight size={16}/></button>
            </div>
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => mutate(rows => {
                ...
                find(rows);
              })} className="text-indigo-400 hover:text-indigo-700 p-1" title="아래로 세로 분할"><Rows size={18}/></button>
              
              {row.cells.length > 1 && (
                <button onClick={() => mutate(rows => {
                  const findAndRemove = (arr: any[]) => arr.forEach(r => {
                    const idx = r.cells.findIndex((c: any) => c.id === cell.id);
                    if (idx > -1) r.cells.splice(idx, 1);
                    else r.cells.forEach((c: any) => { if(c.nestedRows) findAndRemove(c.nestedRows); });
                  });
                  findAndRemove(rows);
                })} className="text-rose-300 hover:text-rose-600 p-1 transition-colors" title="현재 열(Cell) 삭제"><Trash2 size={16}/></button>
              )}
            </div>
          </div>
      ...
      <button onClick={() => mutate(rows => {
        ...
        find(rows);
      })} className="w-12 shrink-0 flex items-center justify-center bg-indigo-600 text-white rounded-2xl shadow-md hover:bg-indigo-700 transition-colors hover:scale-105" title="옆으로 열(Column) 분할"><Columns size={20}/></button>
    </div>
  );
            ...
            <div className="p-6 border-b flex justify-between items-center bg-slate-50">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-black text-slate-800">
                  ...
                  <span className="text-sm text-indigo-600 ml-3 bg-indigo-100 px-3 py-1 rounded-full">{previewData.length}건 조회됨</span>
                </h3>
              </div>
              ...
              {isLoadingPreview ? (
                ...
                <table className="w-full text-left border-collapse bg-white">
                  <thead className="bg-slate-900 text-white sticky top-0 z-10">
                    <tr>{availableColumns.map(col => <th key={col} className="p-4 text-xs font-black uppercase tracking-wider border-b border-slate-700 whitespace-nowrap">{col}</th>)}</tr>
                  </thead>
                  <tbody>
                    {previewData.map((row, idx) => (
                      <tr key={idx} className="border-b border-slate-100 hover:bg-indigo-50/50 transition-colors">
                        {availableColumns.map(col => <td key={col} className="p-4 text-sm font-medium text-slate-700 whitespace-nowrap max-w-[200px] truncate">{row[col] !== null ? String(row[col]) : <span className="text-slate-300 italic">null</span>}</td>)}
                      </tr>
                    ))}
                  </tbody>
                </table>
